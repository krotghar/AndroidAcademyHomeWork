...
class MovieViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(content: MovieCardContent) {
        genres?.text = content.genres
        pg?.text = content.pg
        bgCard?.setImageResource(content.bgRef)
        movieTitle?.text = content.titleMovie
        reviews?.text = content.reviews
        time?.text = content.time
        ratingBar?.rating = content.rating
    }
}