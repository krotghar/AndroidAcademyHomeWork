...
class MovieDetailsFragment : Fragment() {
    ...
    override fun onAttach(context: Context) {
        ...
        if (context is MovieDetailsBackClickListener) listener = context
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<RecyclerView>(R.id.recycler_actors).apply {
            ...
            this.adapter = ActorsRecyclerAdapter()
        }
    ...
    private fun showMovieNotFoundError() = Toast.makeText(requireContext(), R.string.error_movie_not_found, Toast.LENGTH_LONG).show()


    private fun bindUI(view: View, movie: Movie) {
        updateMovieDetailsInfo(movie)
        val adapter = view.findViewById<RecyclerView>(R.id.recycler_actors).adapter as ActorsRecyclerAdapter
    ...
    private fun updateMovieDetailsInfo(movie: Movie) {
        context?.let { view?.findViewById<ImageView>(R.id.movie_logo_image)?.let { it1 ->
            Glide.with(it).load(movie.detailImageUrl).into(
                it1
            )
        } }
